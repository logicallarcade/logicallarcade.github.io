...
function renderLobby(presenceState) {
    ...
    if (activeRooms.length === 0) {
        ...
        container.innerHTML = activeRooms.map(room => {
            ...
            const isTyperace = gameKey === 'typerace';

            const gameLabel = isWordle ? 'Wordle' : (isSudoku ? 'Sudoku' : (isBingo ? 'Bingo' : 'Type Race'));
            const gameIconHtml = isWordle 
                ? '<i class="fa-solid fa-font"></i>' 
                : (isSudoku ? '<span class="font-black font-mono tracking-tighter text-[10px] mr-1">123</span>' : (isBingo ? '<i class="fa-solid fa-circle-dot"></i>' : '<i class="fa-solid fa-keyboard"></i>'));
            const iconBg = isWordle ? 'border-emerald-500/30 bg-emerald-500/10' : (isSudoku ? 'border-brandPurple/30 bg-brandPurple/10' : (isBingo ? 'border-cyan-500/30 bg-cyan-500/10' : 'border-amber-500/30 bg-amber-500/10'));
            const iconColor = isWordle ? 'text-emerald-400' : (isSudoku ? 'text-brandPurple' : (isBingo ? 'text-cyan-400' : 'text-amber-400'));
            const badgeClass = isWordle
                ? 'lobby-badge--wordle'
                : (isSudoku ? 'lobby-badge--sudoku' : (isBingo ? 'lobby-badge--bingo' : 'lobby-badge--typerace'));

            const maxPlayers = room.maxPlayers || (isWordle ? 2 : (isSudoku ? 5 : (isBingo ? 8 : 4)));
